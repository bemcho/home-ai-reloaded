...
public:
	...
	void loadTESSERACTModel(const char* dataPath, const char* lang, tesseract::OcrEngineMode mode = tesseract::OEM_DEFAULT);
	...
	void predictWithLBP(vector<Annotation>& annotations, vector<Rect> detects, cv::Mat & frame_gray);
	...
	void predictWithTESSERACT(vector<Annotation>& annotations, cv::Mat & frame_gray);
	void predictWithTESSERACT(vector<Annotation>& annotations, vector<Rect> detects, cv::Mat & frame_gray);
	Annotation predictWithTESSERACTInRectangle(const Rect & detect, Mat & frame_gray);

private:
	CascadeClassifier cascade_classifier;
	Ptr<face::FaceRecognizer> model;
	shared_ptr<tesseract::TessBaseAPI> tess;
	double maxDistance;
	dnn::Net net;
